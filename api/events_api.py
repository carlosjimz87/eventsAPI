...
class EventsApi:
    ...
    def get_available_events(
            ...
            ends_at: Union[str, datetime]) -> List[EventSummary]:
        ...
        for date in query_dates:
            events = EventsApi.get_events_on_date(date)

        eventList = [
            event for event in events if event["sell_mode"] == "online"]

        return eventList

    @staticmethod
    def get_events_on_date(date) -> EventList:
        events: EventList = []

        response = FakeProvider.events_on_date(date)
    ...
    @staticmethod
    def get_query_dates(ends_at, starts_at):
